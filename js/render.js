...
function cardHTML(t) {
  ...
  const dueText = t.due
    ? `<p class="card__due${overdue ? " card__due--overdue" : ""}">${icon("calendar")}${overdue ? "Overdue since" : "Due"} ${formatDate(t.due)}</p>`
    : "";
  ...
  return `
    <article class="card${t.status === "done" ? " card--done" : ""}" data-id="${t.id}">
      <div class="card__top">
        ...
        <button class="icon-btn" type="button" data-action="menu" data-id="${t.id}" aria-haspopup="menu" aria-expanded="false" aria-label="More actions for ${escapeHtml(t.title)}">${icon("more-horizontal")}</button>
      </div>
      ...
      <div class="card__foot">
        ${dueText || "<span></span>"}
       <button class="check" type="button" data-action="toggle" data-id="${t.id}" aria-label="${t.status === "done" ? "Reopen" : "Mark complete"}: ${escapeHtml(t.title)}">${icon("check")}</button>
      </div>
    </article>`;
}
...
function emptyHTML(status, filtering) {
  if (filtering)
    return `<div class="empty"><h3>Nothing here</h3><p>No matching tasks in this column.</p></div>`;
  const [title, text] = EMPTY[status];
  const cta =
    status === "todo"
      ? `<button class="btn btn--primary" type="button" data-action="add" data-status="todo">${icon("plus")}Add task</button>`
      : "";
...
function renderStats() {
  ...
  document.querySelector("#stats").innerHTML = [
    ...
    .map(
      ([label, n]) => `<div class="stat"><dt>${label}</dt><dd>${n}</dd></div>`,
    )
    .join("");
}
...
function renderBoard() {
  ...
  STATUSES.forEach((status) => {
    ...
    document.querySelector(`[data-count="${status}"]`).textContent =
      list.length;
    document.querySelector(`[data-body="${status}"]`).innerHTML = list.length
      ? list.map(cardHTML).join("")
      : emptyHTML(status, filtering);
  });
...
function renderBanner() {
  ...
  <div class="banner banner--warn" role="status">
    ${icon("alert-triangle")}
    <span>Changes aren't being saved — this browser's storage isn't available right now.</span>
