...
export function openMenu(id, anchorEl) {
  ...
  menu.innerHTML = `
    <button type="button" role="menuitem" data-m="open">View details</button>
    <button type="button" role="menuitem" data-m="edit">Edit</button>
    ${['todo', 'progress', 'done'].map((s) =>
      `<button type="button" role="menuitem" data-m="move:${s}">Move to ${STATUS_LABEL[s]}</button>`
    ).join('')}
    <hr>
    <button type="button" role="menuitem" data-m="delete" class="menu__danger">Delete</button>`;
...
document.addEventListener('click', (e) => {
  if (!menu.hidden && !menu.contains(e.target) && !e.target.closest('[data-action="menu"]')) closeMenu(false);
});