...
function renderDetails() {
  ...
  <button class="btn btn--danger" type="button" data-d="delete">${icon("trash")}Delete</button>
  <button class="btn btn--secondary" type="button" data-d="edit">${icon("pencil")}Edit</button>
  <button class="btn btn--primary" type="button" data-d="next">${icon("arrow-right")}${next.label}</button>
</div>`;
}
...
body.addEventListener("click", (e) => {
  ...
  if (action === "edit") {
    dialog.close();
    openTaskForm("todo", currentId);
  } else if (action === "delete") {
    dialog.close();
    removeTaskWithUndo(currentId);
  } else if (action === "next") {
